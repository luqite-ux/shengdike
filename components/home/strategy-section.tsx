...
const strategyIcons = [Settings, Maximize2, Zap, ShieldCheck, Headphones, Award]

export function StrategySection() {
  const m = useSiteMarketing()
  const s = m.homeStrategy
  ...
  return (
    <section ref={ref} className="py-20 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900">{s.sectionTitle}</h2>
          <div className="w-16 h-1 bg-[#E94709] mx-auto mt-4" />
          <p className="mt-6 text-gray-600">{s.sectionSubtitle}</p>
        </motion.div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 bg-[#1a1a2e] rounded-xl overflow-hidden">
          {s.cards.map((strategy, index) => {
            const Icon = strategyIcons[index] ?? Settings
            return (
              <motion.div
