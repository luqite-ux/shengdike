...
const { siteSettingsDoc, homePageDoc, aboutPageDoc, companyProfilePageDoc, siteMarketingContentDoc } =
  getSeedDocuments();
...
async function main() {
  ...
  tx.createOrReplace(companyProfilePageDoc);
  tx.createOrReplace(siteMarketingContentDoc);
  const res = await tx.commit();
  console.log(
    '[seed] 已写入（或覆盖）siteSettings、homePage、aboutPage、companyProfilePage、siteMarketingContent 的已发布文档。',
  );
